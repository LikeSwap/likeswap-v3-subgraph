 ...
 * @param event
 */
export function updateLikeswapDayData(event: ethereum.Event): LikeswapDayData {
  let likeswap = Factory.load(FACTORY_ADDRESS)
  let timestamp = event.block.timestamp.toI32()
  let dayID = timestamp / 86400 // rounded
  let dayStartTimestamp = dayID * 86400
  let likeswapDayData = LikeswapDayData.load(dayID.toString())
  if (likeswapDayData === null) {
    likeswapDayData = new LikeswapDayData(dayID.toString())
    likeswapDayData.date = dayStartTimestamp
    likeswapDayData.volumeETH = ZERO_BD
    likeswapDayData.volumeUSD = ZERO_BD
    likeswapDayData.volumeUSDUntracked = ZERO_BD
    likeswapDayData.feesUSD = ZERO_BD
  }
  likeswapDayData.tvlUSD = likeswap.totalValueLockedUSD
  likeswapDayData.txCount = likeswap.txCount
  likeswapDayData.save()
  return likeswapDayData as LikeswapDayData
}
